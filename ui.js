...
import { getPpn, itemNetPrice, shopStockOf, getActiveShopId, getShops } from './storage.js';
...
function renderTaxReport(d) {
  ...
  return `
    ...
    ${!d.pphEligible ? `<div style="background:#fef2f2;border:1px solid #fecaca;color:#b91c1c;border-radius:10px;padding:10px;font-size:12px;margin-bottom:10px">⚠️ <b>Omzet ${d.year} melebihi Rp4,8 M</b> — PPh Final 0,5% (PP 23/2018) tidak lagi berlaku untuk seluruh omzet. Gunakan tarif umum &amp; pertimbangkan kewajiban PKP. Konsultasikan dengan konsultan pajak sebelum melapor.</div>` : ''}
...
function variantCombos() {
  ...
  return { sizes, colors, S: sizes.length ? sizes : [''], C: colors.length ? colors : [''] };
}
export function setStockVariantMode(mode) {
  stockVariantMode = mode === 'variant' ? 'variant' : 'single';
  applyVariantMode();
}
export function getStockVariantMode() { return stockVariantMode; }
export function applyVariantMode() {
...
function bindVariantRupiah() {
  document.querySelectorAll('.vprice, .vcost').forEach(el => {
...
export function updateVariantTotal() {
  ...
  if (!el) return;
  const stocks = [...document.querySelectorAll('.vstock')].map(i => Math.max(parseInt(i.value || '0', 10) || 0, 0));
