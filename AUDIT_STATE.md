...
Repo **v1.76.0** · Production **v1.76.0 VERIFIED 2026-09-14** di `https://wynara-acc.vercel.app` (check-prod PASS). Backend Supabase **LIVE (server-authoritative)**.
...
A+ requires every axis ≥85 **and** overall ≥95. ALL nine axes ≥85; overall 92.98 still <95. Lowest: F8/F9 85.
```
...
| F2 Tax conformance | 12 | 76 | **89** | 10.68 | + COA e-wallet/detail + aturan bank berarah berbasis contoh transaksi & biaya |
...
| F7 Cognitive load | 10 | 80 | **97** | 9.70 | Aturan Bank tabel + inline edit + cari + preset sekali-klik; dompet terkelompok; wizard |
...
| **OVERALL** | | ~~90~~ | | **92.98** | v1.76 UX Aturan Bank (tabel/inline/edit) + Kas & Bank terkelompok; arithmetic di bawah |

Aritmetika (wajib tampil): 99×15 + 89×12 + 86×12 + 93×15 + 97×10 + 99×12 + 97×10 + 85×7 + 85×7
= 1485 + 1068 + 1032 + 1395 + 970 + 1188 + 970 + 595 + 595 = **9298 / 100 = 92.98**. Baseline 59.7 → **+33.28**.
...
4. **Akuntan/HRD permission matrix** — **RESOLVED + IMPLEMENTED 2026-09-13 (v1.41.0)**: Akuntan = semua akuntansi (ledger+payroll+data+settings); HRD = gaji + penggantian kas kecil (buat entri); PIN per peran; `requireCap()` di storage. → F4 74→82.
...
- **v1.46.0 (F4)**: **🔌 Uji koneksi** cloud (ping REST 1 baris → cek sesi + RLS) + status sinkron menampilkan **jumlah konflik**. **→ F4 82→84.** +1 test. 252/252 ✓.

- **v1.47.0 (F2)**: **CSV rekap PPh 21** (e-SPT 21) + banner **⏰ Tenggat terdekat** di laporan Pajak. **→ F2 83→85.** 252/252 ✓.
...
- **v1.51.0**: **Masuk dengan email** (pulihkan data dari cloud saat ganti domain/HP) — `cloudSignIn` + `syncNow`. +1 test.

- **v1.52.0 (F1/F6)**: **Multi-toko** — Pengaturan kelola toko; item menyimpan stok **per toko** (`stocks{}`), total = jumlah; pilih **toko aktif** di halaman Stok; jual/beli/restock/opname pada toko aktif; kartu stok mencatat toko; daftar toko ikut sinkron & backup. **→ (bagian dari kenaikan F1/F6).** +2 test. 258/258 ✓.

- **v1.53.0 (F1/F6 — Fase 1 stok)**: **Satuan/Kategori/Barcode** produk; **SKU & barcode per varian**; **nilai persediaan** + jumlah **perlu restock** per toko; **scan barcode/SKU** (Enter buka produk); tombol **📜 Riwayat** (kartu stok gabungan varian); import memetakan kolom baru. **→ F1 90→91, F6 90→91.** +1 test. 259/259 ✓.

- **v1.54.0 (F1/F6 — Fase 2a POS sale)**: kotak **scan/cari** (barcode/SKU/nama + Enter → keranjang, qty bertambah bila sudah ada), tombol **qty −/＋**, **diskon nota (Rp)** dengan ringkasan Subtotal→Diskon→PPN→Total + estimasi untung; cek stok per **toko aktif**. 259/259 ✓.

- **v1.55.0 (F1/F6 — Fase 2b dokumen stok)**: **⚖️ Penyesuaian stok** (+/− dengan alasan → jurnal Dr/Cr 1301 vs 5199, hormati kunci periode) & **🔁 Transfer antar toko** (internal, rollback aman). **→ F1 91→92, F6 91→92.** +2 test. 261/261 ✓.

- **v1.56.0 (F1/F6)**: **tampilan tabel** sortable + **aksi massal** (arsip/aktif, set kategori, export CSV) + **laporan perlu restock**. **→ F1 92→93, F6 92→93.** +1 test. 262/262 ✓.

- **v1.57.0 (F1/F2 — Fase 3 retur)**: **retur penjualan parsial** per baris — stok balik, jurnal Dr 4101/2105 + Dr 1301, Cr kas + Cr 5109; refund via metode; tersimpan & sinkron. **→ F1 93→94, F2 85→86.** +2 test. 264/264 ✓.
